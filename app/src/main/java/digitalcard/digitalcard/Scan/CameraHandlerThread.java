...
public class CameraHandlerThread extends HandlerThread {
    ...
    public void startCamera(final int cameraId) {
        ...
        localHandler.post(new Runnable() {
            @Override
            public void run() {
                final Camera camera = digitalcard.digitalcard.Scan.CameraUtils.getCameraInstance(cameraId);
                Handler mainHandler = new Handler(Looper.getMainLooper());
                mainHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        mScannerView.setupCameraPreview(digitalcard.digitalcard.Scan.CameraWrapper.getWrapper(camera, cameraId));
                    }
                });
            }
        });
